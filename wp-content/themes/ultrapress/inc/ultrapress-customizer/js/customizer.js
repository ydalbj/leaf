jQuery( document ).ready(function($) {
		...
		if(numRepeaterItems > 0) {
			...
			if(numRepeaterItems > 1) {
				...
				for (i = 1; i < numRepeaterItems; ++i) {
					ultrapressAppendRow($(this), defaultValuesArray[i]);
				}
			}
		}
	});
	...
	$(this).find('.sortable_repeater.sortable').sortable({
		update: function(event, ui) {
			ultrapressGetAllInputs($(this).parent());
		}
	});
	...
	$('.sortable_repeater.sortable').on('click', '.customize-control-sortable-repeater-delete', function(event) {
		...
		if(numItems > 1) {
			$(this).parent().slideUp('fast', function() {
				...
				$(this).remove();
				ultrapressGetAllInputs(parentContainer);
			})
		}
		else {
			$(this).parent().find('.repeater-input').val('');
			ultrapressGetAllInputs($(this).parent().parent().parent());
		}
	});
	// Add new item
	$('.customize-control-sortable-repeater-add').click(function(event) {
		event.preventDefault();
		ultrapressAppendRow($(this).parent());
		ultrapressGetAllInputs($(this).parent());
	});
	...
	function ultrapressAppendRow($element, defaultValue = '') {
	...
	function ultrapressGetAllInputs($element) {
	...
	$('.multi-image-checkbox').on('change', function () {
	  ultrapressGetAllImageCheckboxes($(this).parent().parent());
	});
	// Get the values from the checkboxes and add to our hidden field
	function ultrapressGetAllImageCheckboxes($element) {
	...
	$( ".pill_checkbox_control .sortable" ).sortable({
		...
		update: function( event, ui ) {
			ultrapressGetAllPillCheckboxes($(this).parent());
		}
	});
	$('.pill_checkbox_control .sortable-pill-checkbox').on('change', function () {
		ultrapressGetAllPillCheckboxes($(this).parent().parent().parent());
	});
	// Get the values from the checkboxes and add to our hidden field
	function ultrapressGetAllPillCheckboxes($element) {
	...
	$('.google-fonts-list').on('change', function() {
		...
		var indexes = $.map(bodyfontcontrol.ultrapressfontslist, function(obj, index) {
		...
		$.each(bodyfontcontrol.ultrapressfontslist[index].variants, function(val, text) {
		...
		$(this).parent().parent().find('.google-fonts-category').val(bodyfontcontrol.ultrapressfontslist[index].category);
		ultrapressGetAllSelects($(this).parent().parent());
	});
	$('.google_fonts_select_control select').on('change', function() {
		ultrapressGetAllSelects($(this).parent().parent());
	});
	function ultrapressGetAllSelects($element) {
		var selectedFont = {
	...
	$('.customize-control-tinymce-editor').each(function(){
		...
		var tinyMCEToolbar1String = _wpCustomizeSettings.controls[$(this).attr('id')].ultrapresstinymcetoolbar1;
		var tinyMCEToolbar2String = _wpCustomizeSettings.controls[$(this).attr('id')].ultrapresstinymcetoolbar2;
		var tinyMCEMediaButtons = _wpCustomizeSettings.controls[$(this).attr('id')].ultrapressmediabuttons;
 ...
 */
( function( $, api ) {
	api.sectionConstructor['ultrapress-upsell'] = api.Section.extend( {
